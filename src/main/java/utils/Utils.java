    ...
    public static List<RawData> parseJson(String str, long currentTime){
        ...
        try {
            ...
            for (Object each: records){
                ...
                ls.add(new RawData(currentTime, Long.parseLong(jobj.get("i").toString()), Double.parseDouble(jobj.get("voltage").toString()), Double.parseDouble(jobj.get("current").toString())));
            }
            return ls;
        }catch (Exception e){
            e.printStackTrace();
